...
export const Home = () => {
  const { getAllProperties, getByCategory, getPropByInterval } =
  ...
  const handleUserFilter = async () => {
    ...
    const teste = await axios.post(
      "http://10.0.4.41:8000/properties/filterprops",
      dataFilter
    );
  ...
  const handleApplyFilter = async () => {
    if (Number(dateInitial) > Number(dateFinal)) {
      return Alert.alert(
        "",
        "Data de entrada deve ser maior ou igual à saída."
      );
    }
    ...
    if (dateInitial == "" || dateFinal == "") {
      ...
      if (
        result.message ==
        "Não foram encontrados imóveis disponíveis neste intervalo."
      ) {
  ...
  return (
    <Container>
      <AreaName>
        ...
        <ImageUser
          source={
            userDataStoraged.foto != null
              ? { uri: `data:image/png;base64,${userDataStoraged.foto}` }
              : ImageUserProfile
          }
        />
      </AreaName>
      ...
      <Header>
        <BtnFilter onPress={handleFilterActived}>
          <Octicons name="gear" size={RFValue(20)} color={theme.colors.white} />
        </BtnFilter>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <CardCategory
              ...
              actived={categorySelected == item.title}
            />
          )}
        />

        {allProps == [] && <txtNotData>Opss, não encontramos</txtNotData>}
      </View>
        ...
        >
          ...
          >
            <ButtonPrice
              ...
              onPress={() => setButtonActived("Maior")}
            >
              <TxtPrice
                style={{
                  color:
            ...
            <ButtonPrice
              ...
              onPress={() => setButtonActived("Menor")}
            >
              <TxtPrice
                style={{
                  color:
